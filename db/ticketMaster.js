...
const callAPI = () => {
  ...
  return axios
    .get(
      `https://app.ticketmaster.com/discovery/v2/events?apikey=${process.env.API_KEY_TICKETMASTER}&latlong=${latitude},${longitude}&radius=50&unit=miles&locale=*&startDateTime=${dates.currentDateStr}&endDateTime=${dates.futureDateStr}&size=100`
    )
    .then(response => {
